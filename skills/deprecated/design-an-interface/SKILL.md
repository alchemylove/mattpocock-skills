...
description: 並列の sub-agent を使って、モジュールの根本的に異なる複数のインターフェース設計を生成する。ユーザーが API を設計したい、interface の選択肢を探りたい、モジュール形状を比較したい、または「design it twice」に言及したときに使う。
---
...
確認: 「この module は何をする必要があるか? 誰がそれを使うか?」
...
- 「どの design が主要な use case に最も合うか?」
- 「他の design から取り入れる価値のある要素はあるか?」
